var m=async()=>await(async f=>{for(const{Path:r,Name:s,File:w}of f)for(const[l,d]of await(await import("../Function/Directory.js")).default(await(await import("../Function/Package.js")).default("NPM"))){const i=`${l}/.github`,e=await w();if(r==="/workflows/"&&s==="NPM.yml")for(const a of d){const t=(await import("node:path")).dirname(a).replace(l,""),y=(await(await import("node:fs/promises")).readFile(a,"utf-8")).toString(),p=(await(await import("../Function/Type.js")).default()).get(a.split("/").pop());try{if(typeof p<"u"&&p==="NPM"){const o=JSON.parse(y);for(const c in o)if(Object.prototype.hasOwnProperty.call(o,c)){const u=o[c];if(c==="scripts")for(const n in u)Object.prototype.hasOwnProperty.call(u,n)&&(n==="build"&&e.add(`
            - name: Publish .${t}
