...
`),n==="Build"&&t.add(`
            - name: Publish .${e}
              ...
              env:
                  NODE_AUTH_TOKEN: \${{ secrets.NPM_TOKEN }}
`))}}}catch(r){console.log(`Could not create: ${a}`),console.log(r)}}if(t.size>1){try{await(await import("node:fs/promises")).mkdir(`${i}${o}`,{recursive:!0})}catch{console.log(`Could not create: ${i}${o}`)}try{await(await import("node:fs/promises")).writeFile(`${i}${o}${s}`,`${[...t].join("")}`)}catch{console.log(`Could not create workflow for: ${i}/workflows/NPM.yml`)}}}})((await import("../Variable/NPM.js")).default);export{m as default};
