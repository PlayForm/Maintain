var y=async()=>await(async f=>{for(const{Path:c,Name:p,File:w}of f)for(const[a,m]of await(await import("../Source/Function/Directory.js")).default(await(await import("../Source/Function/Package.js")).default("NPM"))){const s=`${a}/.github`,t=await w();if(c==="/workflows/"&&p==="Node.yml")for(const o of m){const e=(await import("node:path")).dirname(o).replace(a,""),$=(await(await import("node:fs/promises")).readFile(o,"utf-8")).toString(),d=(await(await import("../Source/Function/Type.js")).default()).get(o.split("/").pop());try{if(typeof d<"u"&&d==="NPM"){const r=JSON.parse($);for(const i of["bundledDependencies","bundleDependencies","dependencies","devDependencies","extensionDependencies","optionalDependencies","peerDependencies","peerDependenciesMeta"].sort())typeof r[i]<"u"&&t.add(`
