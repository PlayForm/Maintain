...
`),n==="test"&&t.add(`
            - run: pnpm run test
              working-directory: .${e}
`))}}}catch(o){console.log(`Could not create: ${a}`),console.log(o)}}let l="main";try{await(await import("node:fs/promises")).access(r,(await import("node:fs/promises")).constants.F_OK);const a=process.cwd();process.chdir(r),l=(await import("child_process")).execSync("git rev-parse --abbrev-ref HEAD").toString().trim(),process.chdir(a)}catch{console.log(`Could not access: ${r}`)}if(t.size>1){try{await(await import("node:fs/promises")).mkdir(`${s}${c}`,{recursive:!0})}catch{console.log(`Could not create: ${s}${c}`)}try{await(await import("node:fs/promises")).writeFile(`${s}${c}${p}`,`${[...t].join("")}`.replaceAll("$Branch$",l))}catch{console.log(`Could not create workflow for: ${s}/workflows/Node.yml`)}}}})((await import("../Variable/Node.js")).default);export{$ as default};
