...
- Added `Maintain Workflow --Review`: read-only, repository-agnostic review of
  the GitHub Actions references in the current repository. Reports every `uses:`
  action with its current reference, the latest tag/commit SHA on GitHub
  (current / stale / floating), and what the canonical `Workflow/` templates
  would apply in absorb phase 2 - writes nothing.
- Added `Maintain Workflow --Absorb`: two-phase, versions-only updater for
  pre-existing repositories. Phase 1 writes a scoped `Update.sh` into the
  repository root that pins every `uses:` reference under `.github/` to the
  latest tag + commit SHA; phase 2 (re-run after executing `Update.sh`) iterates
  the canonical `Workflow/` templates and applies their pinned versions in
  place - only the `uses:` version segment is replaced and never downgraded;
  everything else is left byte-for-byte intact.

### Fix

- `Update.sh`: replaced GNU-only `\s` with POSIX `[[:space:]]` in the `uses:`
  rewrite, which silently never matched under BSD sed (macOS) - version pins
  were never updated.
- `Update.sh`: byte-preserving line handling so a workflow file whose last line
  lacks a trailing newline is no longer truncated by `while read`.
